...
export class RagVectorService {
	...
	async initialize(root: string): Promise<void> {
		this.backend = 'local';
		try {
			const raw = await fs.readFile(path.join(root, '.aqiron-security', 'vectors.json'), 'utf8');
	...
	async rebuild(root: string, chunks: RagChunk[]): Promise<void> {
		...
		const directory = path.join(root, '.aqiron-security');
		await fs.mkdir(directory, { recursive: true });
		await fs.writeFile(path.join(directory, 'vectors.json'), JSON.stringify([...this.vectors].map(([id, values]) => ({ id, values }))), 'utf8');
	}
	...
	private searchFaiss(query: number[], limit: number): Map<string, number> {
		...
		try {
			...
			for (let index = 0; index < (result.labels?.length ?? 0); index++) {
				...
				if (id) {scores.set(id, 1 / (1 + (result.distances?.[index] ?? 0)));}
			}
			return scores;
		} catch {
...
function createFaissIndex(entries: Array<[string, number[]]>): RagVectorService['faiss'] | undefined {
	try {
		...
		if (!native.IndexFlatL2 || entries.length === 0) {return undefined;}
